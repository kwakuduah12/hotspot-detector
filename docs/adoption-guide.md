...
- **First onboard skip (old behavior):** [PR #6](https://github.com/kwakuduah12/hotspot-detector/pull/6) added the export hotspot while matching used only the merge-base manifest, so the gate quiet-skipped.
- **First onboard runs:** closed [PR #14](https://github.com/kwakuduah12/hotspot-detector/pull/14) added `store.py` as a new hotspot path after union matching landed. CI ran `ingest_bulk` and posted last good vs this PR (`ok`). Do not merge that change; `store.py` is shared plumbing, not a top profile frame.
