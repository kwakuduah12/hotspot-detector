"""Workload: seed then allocation-heavy query scan. Prints timings as JSON."""
...
def run(base_url: str | None = None) -> list[dict]:
    ...
    with httpx.Client(timeout=60.0) as client:
        ...
        resp = client.get(f"{base}/query", params={"tag": "alpha"})
        resp.raise_for_status()
        scan_ms = (time.perf_counter() - t0) * 1000.0
        body = resp.json()
        if body.get("count", 0) < 1:
            raise RuntimeError("query returned no records")

    return [{"operation": "scan", "duration_ms": round(scan_ms, 3)}]
